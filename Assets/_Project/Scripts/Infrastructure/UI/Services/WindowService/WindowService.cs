...
namespace CodeBase.Infrastructure.UI.Services
{
    public class WindowService : IWindowService
    {
        ...
        public TWindow GetOpenedWindow<TWindow>() where TWindow : WindowBase
        {
            if (!_openedWindows.ContainsKey(typeof(TWindow)))
            {
                Debug.LogError($"Window with name {typeof(TWindow)} not opened");
                return default;
            }

            return (TWindow)_openedWindows[typeof(TWindow)];
